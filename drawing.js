...
// New 8-bit style futuristic fighter with dual thrusters & angled wings
export function drawPlayer8Bit(ctx, player) {
  // 9x9 pixel grid: 0=empty, 1=hull, 2=cockpit, 3=accent
  const grid = [
    "001010100", // nose tip
    ...
    "010010010"  // exhaust glow for left/right thrusters
  ];
  ...
  for (let gy = 0; gy < pixels; gy++) {
    ...
    for (let gx = 0; gx < row.length; gx++) {
      ...
      let color = hullColor;
      if (v === "2") color = cockpitColor;
      if (v === "3") color = accentColor;
      if (gy === 8 && v === "1") color = exhaustColor;
  ...
  if (player.thrusting) {
    ...
    const thrusterOffset = size / 4;
    // Left thruster
    ctx.beginPath();
    ctx.ellipse(player.x - thrusterOffset, player.y + size / 2.6, size / 6, size / 8, 0, 0, Math.PI * 2);
    ctx.fill();
    // Right thruster
    ctx.beginPath();
    ctx.ellipse(player.x + thrusterOffset, player.y + size / 2.6, size / 6, size / 8, 0, 0, Math.PI * 2);
